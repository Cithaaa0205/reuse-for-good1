...
<div class="max-w-5xl mx-auto">
    ...
    {{-- WRAPPER NAV + KONTEN TAB (SATU x-data) --}}
    <div x-data="{ activeTab: 'donasi' }" x-cloak>
                ...
                <nav class="flex -mb-px space-x-6">
                    ...
                    <a href="#donasi"
                       ...
                       :class="{
                            ...
                            'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-400': activeTab !== 'donasi'
                        }"
                       class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                        Barang Didonasikan
                    </a>

                    {{-- Tab Privat: hanya kalau lihat profil sendiri --}}
                    @if(Auth::check() && Auth::id() == $user->id)
                        <a href="#diterima"
                           @click.prevent="activeTab = 'diterima'"
                           :class="{
                                'border-blue-600 text-blue-600': activeTab === 'diterima',
                                'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-400': activeTab !== 'diterima'
                            }"
                           class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                            Barang Diterima
                        </a>
                        ...
                        <a href="#favorit"
                           ...
                           :class="{
                                ...
                                'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-400': activeTab !== 'favorit'
                            }"
                           class="whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200">
                            Favorit
                        </a>
        ...
        <div>
            ...
            {{-- TAB 2 & 3 HANYA UNTUK PEMILIK PROFIL --}}
            @if(Auth::check() && Auth::id() == $user->id)
                ...
                <div x-show="activeTab === 'favorit'">
                    ...
                    @else
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            @foreach($favorites as $item) {{-- $item = BarangDonasi --}}
                                <div class="bg-white rounded-xl shadow overflow-hidden">
                                    <a href="{{ route('barang.show', $item->id) }}">
                                        @if($item->foto_barang_utama)
                                            <img src="{{ asset('uploads/barang/' . $item->foto_barang_utama) }}"
